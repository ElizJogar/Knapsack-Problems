...
namespace GenAlgorithm
{
    ...
    class ParametersCombinationsAnalysisReport : AExcelReport
    {
        ...
        public ParametersCombinationsAnalysisReport(int iterationCount, int populationCount, int betta, int startCount, GenAlgorithm algorithm)
        {
                ...
                for (int j = 0; j < crossover.Length; j++)
                    for (int k = 0; k < mutation.Length; k++)
                        for (int g = 0; g < selection.Length; g++)
                        {
                            ...
                            {
                                convergence += "IF(" + vsS[number - 1] + 2 +/* "=Sheet" + v*/workbook.Worksheets.get_Item(v) + "!" + vsS[number - 1] + (_iterationCount + 4) + "," + "Sheet" + v + "!" + vsS[number - 1] + (_iterationCount + 6) + "," + 100 + ")" + ",";
                            }
    ...
    {
        private GenAlgorithm _algorithm;
        private ParametersCombinationsAnalysisReport _report;
        private int _iterationCount;
        private int _populationCount;
        private int _betta;
        private int _startCount;
        ...
        public DataInstancesAnalysisReport(int iterationCount, int populationCount, int betta, int startCount, int InstancesCount)
        {
            ...
            _betta = betta;
            _startCount = startCount;
           // _report = new ParametersCombinationsAnalysisReport(iterationCount, populationCount, betta, startCount, algorithm);
           // _algorithm = algorithm;
        }
        public override void Create()
        {
