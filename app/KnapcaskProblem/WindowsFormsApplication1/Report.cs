...
namespace GenAlgorithm
{
    ...
    class ParametersCombinationsAnalysisReport : AExcelReport
    {
        ...
        public ParametersCombinationsAnalysisReport(int iterationCount, int populationCount, int betta, int startCount, ADataInstances data)
        {
            ...
            _dataType = data.GetStringType();
            _algorithm = new GenAlgorithm(data);
        ...
        public override void Create()
        {
            ...
            _excel.Visible = true;
            workbook.Saved = true;
            workbook.SaveCopyAs(_dir + @"\" + _dataType + "xlsx");
            workbook.Close();
            _excel.Quit();
        }
    }
    ...
    class DataInstancesAnalysisReport : AExcelReport
    {
        ...
        public override void Create()
        {
            for (int globIndex = 0; globIndex < 1; globIndex++)
            {
                switch (globIndex)
                {
                    case 0:
                ...
                for (int instIndex = 0; instIndex < _instancesCount; instIndex++)
                {
                    ...
                    _excel.SheetsInNewWorkbook = _startCount + 1;
                    _excel.Workbooks.Add(Type.Missing);
                    Excel.Workbook workbook = _excel.Workbooks[2];
                    Excel.Worksheet sheet = workbook.Worksheets.get_Item(1);
                    for (int s = 0; s < _startCount; s++)
                    {
                        int count = 1;
                    ...
                    sheet.Cells[22, 5] = "=COUNTIF(" + vsS[length / 2 + 1] + "2:" + vsS[length] + "2," + vsS[3] + "13)";
                    workbook.Saved = true;
                    workbook.SaveAs(_dir + @"\" + _data.GetStringType() + "_" + (instIndex + 1) + ".xlsx");
                    (sheet as Excel.Worksheet).Copy(summaryWorkbook.Worksheets[instIndex + 1]);
                    sheet = summaryWorkbook.Worksheets[instIndex + 1];
                    sheet.Cells[23, 1] = "HELLO";
                    workbook.Close();
                }
                summaryWorkbook.SaveAs(_dir + @"\" + _data.GetStringType() + "-summary.xlsx");
                summaryWorkbook.Close();
                MessageBox.Show(@"Report created successfully!", "Excel Tool", MessageBoxButtons.OK, MessageBoxIcon.Information);       
                //_excel.Quit();
            }
        }
    }
